...
//! (short-header) packet on the wire. A naive DPI / GFW UDP classifier sees the
//! QUIC short-header form (fixed bit set, header-form bit clear), a stable
//! per-session Destination Connection ID, and a varying packet number, followed
//! by what looks like a QUIC-protected payload (our `salt ++ AEAD` bytes).
//!
//! This is **cosmetic framing, not a real QUIC stack** — it adds no security and
//! is only meant to evade naive UDP/protocol classification. The peer (server)
//! must apply the exact inverse; the wire contract is documented in DESIGN.md
//! ("QUIC obfs") so a server can be implemented to match.
//!
...
//! ```
//!
//! * **first byte** — header-form bit (`0x80`) CLEAR and fixed bit (`0x40`) SET,
//!   marking a QUIC short header. The low two bits carry `PN_LEN - 1` (QUIC's
//!   packet-number-length field); the middle bits are randomized (in real QUIC
//!   they are header-protected and so look random).
//! * **DCID** — `dcid_len` opaque bytes, generated once per session and reused
//!   for every packet (a real QUIC connection keeps a constant DCID). The
//!   decoder strips them by length without reading them, so both ends only need
//!   to agree on `dcid_len`.
//! * **packet number** — a `PN_LEN`-byte big-endian counter that increments per
//!   packet (cosmetic; never read on decode beyond its length).
//!
...
/// A QUIC short-header obfuscator for one session. Cheap to share via `Arc`.
pub struct QuicObfs {
    /// Per-session Destination Connection ID (constant for the session).
    dcid: Vec<u8>,
...
impl QuicObfs {
    ...
    /// returning the on-wire bytes to send.
    pub fn wrap(&self, datagram: &[u8]) -> Vec<u8> {
        let pn = self.pn.fetch_add(1, Ordering::Relaxed);

        // Randomize the middle bits of the first byte (they are header-protected
        // — and so effectively random — in real QUIC), keeping the form bits and
        // the packet-number-length field deterministic.
        let mut rnd = [0u8; 1];
    ...
    /// datagram slice. Returns `None` if the first byte isn't a QUIC short header
    /// or the packet is too short to hold the prefix (caller drops it).
    pub fn unwrap<'a>(&self, pkt: &'a [u8]) -> Option<&'a [u8]> {
        let first = *pkt.first()?;
        // Must be a short header: header-form bit clear, fixed bit set.
    ...
    /// Size of the prefix this obfuscator emits.
...
impl Obfuscator {
    ...
    /// (Used by the server build; the client constructs from its config enum.)
    #[allow(dead_code)]
...
mod tests {
    ...
    fn base64_round_trips_and_is_printable() {
        ...
        // Non-base64 bytes are rejected (so a mismatched peer's packet drops).
    ...
    fn wrap_then_unwrap_round_trips() {
        ...
        // Prefix present: 1 + 8 (DCID) + 2 (PN) = 11 bytes ahead of the payload.
        assert_eq!(wire.len(), payload.len() + 1 + DEFAULT_DCID_LEN + 2);

        // Looks like a QUIC short header: form bit clear, fixed bit set.
        assert_eq!(wire[0] & 0x80, 0);
        assert_eq!(wire[0] & 0x40, 0x40);

        let inner = obfs.unwrap(&wire).expect("unwrap");
        assert_eq!(inner, payload);
    }

    #[test]
    fn unwrap_decodes_any_pn_length() {
        // A peer may emit a different PN length; decode keys off the low 2 bits.
        let obfs = QuicObfs::new(DEFAULT_DCID_LEN);
        let payload = b"abc";
    ...
    fn unwrap_rejects_non_short_header_and_truncated() {
        let obfs = QuicObfs::new(DEFAULT_DCID_LEN);
        // Long-header form (0x80 set) is rejected.
        assert!(obfs.unwrap(&[0xC0, 0, 0, 0]).is_none());
        // Fixed bit clear is rejected.
